...
> AArch64:libc2.27:ubuntu18.04，题目是在mini_httpd的baisc认证处塞了个栈溢出，远程不是qemu-user，应该是真机，所以不能直接ret2shellcode。故必须要ROP，其实还真是头一次正经做arm64的ROP，使用AArch64通用gadget调用mprotect，再shellcode即可。
...
就把容易出问题的函数挨个都看了一遍，什么read，scanf，sscanf，sprintf，strcpy，最后看到memcpy时赫然一个可能的栈溢出摆在面前：
...
往上跟几个函数，再对着源码找到这里是basic认证处，发现这段的确是自己加的溢出点，传入的长度显然就是base64解码之后的长度：
...
虽然canary基本阻止了栈溢出的利用，但很多底层代码，如芯片ROM，基带，通信模组等很多就是没有canary，原因不详。从这个意义上来讲，这个把返回地址放到函数栈帧的上面的确是一种漏洞的缓解办法。
...
所以天天说qemu-user的不支持NX的解法就翻车了，必然要ROP了，arm64的ROP不太好找，因为寄存器很多，少有数据直接从栈弹到参数寄存器的指令，另外不像x86可以非对齐搞指令，所以从控栈到控参数寄存器硬找gadget有些麻烦，比arm32要复杂：
...
所以无法正常搜索内存，并且不知道咋修，所以直接用树莓派刷了ubuntu18.04的镜像：
...
然后直接gdb，连patch fork都省了，gdb-multiarch设置如下:
