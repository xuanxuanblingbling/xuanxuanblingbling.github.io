...
不过其写入的命令也有些麻烦，根据手册，写入的bit为`ClearMask`，是清0。例如想写入0，则需要把原来的0xff的8个bit全部清零，即255（11111111），所以写入的数据是目标数据取反，也可以看成255减目标数据。而且经过尝试，在命令行中需要填入十进制，不能填写如11111111的二进制字符串（ClearMask的误导），最终命令如下，165 90 165 90为4ByteWriteKey（A5 5A A5 5A）：
