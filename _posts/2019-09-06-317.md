...
并且在这个过程中我们已经控制了RIP，但是没有直接的代码或者函数可以用，所以要不是就是自己写shellcode蹦过去，要不就是ROP。但是程序中没有可写可执行的代码段，我也不知道栈的位置（不知道rsp在哪），虽然我能任意地址写，但我也就没有办法布置栈的内容，也就没有办法实现ROP。但是，我们是控制了RIP的，也许能在某时，我们可以把rsp修改到我们知道的地方，只要再次之前布置好那个位置，然后只要程序返回，我们就可以成功的ROP啦！
...
- 第三件事情，在main函数的结尾我们可以看到汇编leave;retn; leave相当于mov rsp,rbp;pop rbp，所以我们可以把.fini_array[0]指向main函数的结尾处，即0x401C4B
...
write(fini_array,p64(call_fini)+p64(main_addr))
...
write(fini_array,p64(leave_ret)+p64(main_addr))
