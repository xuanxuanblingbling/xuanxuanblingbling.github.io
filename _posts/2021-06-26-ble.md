...
| [Scapy](https://scapy.readthedocs.io/en/latest/layers/bluetooth.html) | 收发，python   | bluez                 | HCI（BR/BLE）    | 收发：使用复杂，可以在python层面直接控制HCI，组织数据包灵活        |
| [pybluez](https://github.com/pybluez/pybluez)                | python   | bluez                 | L2CAP（BR/BLE）  | 收发：比较成熟的python蓝牙库，L2CAP、RFCOMM层次通信接口友好        |
| [bluepy](https://github.com/IanHarvey/bluepy)                | python   | bluez                 | GATT (BLE)       | 收发：GATT相关处理较多，扫描脚本blescan命令行直接可用              |
| [pygatt](https://github.com/peplin/pygatt)                   | python   | gatttool(bluez tools) | GATT (BLE)       | 收发：将gatttool封装成python接口                                   |
