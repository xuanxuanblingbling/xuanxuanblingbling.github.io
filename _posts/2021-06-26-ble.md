...
> 本题考查的层次为BLE的链路层，考查点是BLE广播报文的捕获与解析。
...
完成题目需要与目标建立L2CAP连接，并发送数据，所以可以参考 [pybluez](https://github.com/pybluez/pybluez) 的 [l2capclient.py](https://github.com/pybluez/pybluez/blob/master/examples/simple/l2capclient.py) 示例，建立连接，最终的解题脚本如下：
