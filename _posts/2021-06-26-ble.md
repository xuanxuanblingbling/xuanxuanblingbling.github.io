...
不过原生API的使用不够便捷与轻量，而linux的蓝牙攻击工具是生态最成熟的，基本都是python的库，只有bluez自带的用户态二进制工具除外。所以linux的PC是我们理想的攻击工具，但由于我们大都是使用linux虚拟机，其使用蓝牙设备的方式是和宿主机共享，在这种情况下可能会出现各种各样的问题，所以一般建议linux虚拟机独占一个蓝牙外设。比如我MacBook Pro的本机蓝牙共享给linux虚拟机，虽然可以使用hciconfig看到蓝牙设备，但之后无法正常进行攻击，后来测试我媳妇的惠普windows本中的linux虚拟机共享蓝牙设备就可以正常攻击。
