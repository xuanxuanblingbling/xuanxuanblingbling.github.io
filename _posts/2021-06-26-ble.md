...
对于协议的抽象设计和实现方法有了清晰的认知之后，**就是什么东西实现在哪？怎么实现的？** 我们从开发者的视角切换成攻击者。我们不仅会对漏洞的发现，原理更加明晰，而且还能更清楚的知道，我们如何发动一次攻击：**即在什么协议层次？怎么构造攻击流量？怎么完成这个层次的流量抓取？** 此时我们需要回答一个问题：
...
> 本题考查的层次为BLE链路层，考查点是BLE广播报文的捕获与解析。
...
> 本题考查的层次为经典蓝牙的L2CAP和SDP层，考查点是SDP服务的发现以及L2CAP的协议格式。
...
完成题目需要与目标建立L2CAP连接，并发送数据，所以可以参考python蓝牙的官方库：[pybluez](https://github.com/pybluez/pybluez) 的 [l2capclient.py](https://github.com/pybluez/pybluez/blob/master/examples/simple/l2capclient.py) 示例，建立连接，最终的解题脚本如下：
...
> 本题考查的层次为BLE的GATT层，考查点是GATT业务的猜测分析。
...
完成题目需要与目标完成GATT层次的通信，可以参考 [pygatt](https://github.com/peplin/pygatt) 的 [subscribe_indicate_thermometer_sample.py](https://github.com/peplin/pygatt/blob/master/samples/subscribe_indicate_thermometer_sample.py) 的示例，最终解题脚本如下：
