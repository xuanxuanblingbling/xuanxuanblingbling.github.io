...
主要就是三种架构方案，后两者的最大区别就是蓝牙app的业务代码与蓝牙协议栈是否运行在同一个芯片上，当然这个区别也就会造成两者提供的开发SDK的本质不同，单芯片的SDK主要完成的是编译烧录运行，而双芯片的则主要是完成通信过程。
