...
漏洞发生在hitb_dma_timer函数中，在进行dma buf内存拷贝操作时，没有对客户机传来的dma buf内存地址进行检查，导致可以越界读写dma buf。而本题中，qemu模拟的dma buf的物理地址0x40000是个假地址，即真正的客户机物理地址0x40000对应的就是正常内存（-m 64M），而这个虚假的0x40000只有在通过题目设备hitb的mmio进行对dma参数设置的时候才有用。题目中这个假dma地址0x40000对应的内存就是qemu进程中的一片内存（不在客户机可直接访问的连续内存中），并且在buf后跟了一个函数指针enc，所以通过读写此函数指针即可完成qemu进程的地址信息泄露以及控制流劫持。
