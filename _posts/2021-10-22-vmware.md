...
> 在misty的帮助下完成，方法是使用由VMware提供的自定义gdb命令：**monitor**来查看这些特权寄存器，如 **monitor r cr3**。其实叫特权寄存器并不准确，其主要包括了x86的控制寄存器和系统地址寄存器，但又没有很好的统称，所以就按照他们的属性：只能由运行在ring0级别的特权指令来操控，统称他们为特权寄存器。查看这些寄存器的目的，是为了更彻底的理解操作系统。
...
所以开始也理所当然的认为vmware的调试也应该给我这个结果，然而是只能看到通用寄存器，但发现了用IDA的命令`r cr0`的确可以看到：
...
仍然没用，最后发现其实IDA的寄存器窗口也没有正常显示cr系列寄存器，而是在命令行，用IDA的gdb命令行help发现**Dump hidden register.**。开始以为这个功能是IDA的黑魔法，但实际搜索发现，这个东西是vmware实现的：
