...
> 这题淼哥基本写的差不多了：[QWB-2021-Final:RealWorld MiRouter WriteUp](https://github.com/ReAbout/ctf-writeup/blob/master/qwb-2021-final/mirouter-wp.md)，我补充些淼哥没写的：设备串口开SSH、eCos业务分析、流量分析、多人策略、以及只开一个窗口的exp。另外今年的文章省去了一些基础操作，如果想看新手教学可以看去年思科的那篇：[思科路由器 RV110W CVE-2020-3331 漏洞复现](https://xuanxuanblingbling.github.io/iot/2020/10/26/rv110w/)。与去年相比这次没有考察二进制漏洞的利用，而是一个命令注入漏洞点的触发路径分析，明年再来一个和云侧、app侧结合的，后年再来一个从空口打的，这样IoT的大面就基本考全了。
...
到此应该大概猜出来了，这个函数的注册关联的名字是init，参数应该是data。并且这个data应该是个json：因为之后的解析函数为`blobmsg_parse`，这一套就是ubus的机制，关于ubus之后会单开一遍讲通信的进行分析。分析至此应该明白，netapi将是我们的目标函数`sub_402070`注册到了tbus总线上，所以之后就是怎么调用这个注册到tbus上的服务了。全局搜索tbus会有如下的一些串：
