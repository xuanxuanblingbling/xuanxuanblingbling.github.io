...
发现题目给的docker会将题目转发到本地的60001端口上，为方便调试制作[docker_gdb.patch](https://xuanxuanblingbling.github.io/assets/attachment/riscv/docker_gdb.patch)：将qemu-user的gdb开在了1234端口上并转发出来，故需要调试器介入方可继续运行，用法如下：
...
0de2a597a22a        favourite_architecture_pwn   "/usr/sbin/xinetd -d…"   About an hour ago   Up About an hour    0.0.0.0:60001->60001/tcp   favourite_architecture_pwn_1
...
> 汇编源码版shellcode：远程版，比赛时的自己作品，因为当时不知道多次读文件才能读到libc那行地址，所以当时采用的是用got表泄露libc，所以交互次数有点多。而且利用的地址需要硬编码到shellcode中，所以不方便写成二进制版。虽然现在看起来又臭又长，但是比赛时搞出来已经不错了hhhhhhh
