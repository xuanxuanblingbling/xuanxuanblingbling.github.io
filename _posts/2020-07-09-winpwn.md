...
的确和《0day》中介绍的堆不同了，最终找到一篇文章是详细的讲解了《0day》中没有提到的另外两个时代的文章，而且划分都一样，因为是参考的《0day》，也是对其内容进行了补充：

- [Windows系统下典型堆漏洞产生原理及利用方法研究](https://www.jianshu.com/p/a853040d2804)

读完这篇文章，我们可以知道，其实现在的堆大体上仍然是《0day》中提到的第三个阶段，也在文中着重的介绍了当前堆的结构实现。不过我们知道Windows是没有公开堆管理器的实现，那么文中提到的各种结构体的名称难道也是极客们逆向出来的？当然不是！
...
原来windbg需要配置对应dll符号源文件才能解析相应的结构体，这也说的通。获得符号文件的方法是配置windbg，让他自己去微软官方下载，不过我按照上面的方法在C盘新建一个symbols文件夹，然后在windbg的【file】 -> 【Symbol File Path】中添加：
...
至此我们可以回答：**各种结构体的名称不是极客们逆向出来的，而是微软提供的。** 虽然ntdll这些动态链接库删去了符号表，但是为了方便调试，还是要放出一些符号信息给开发者。这些符号信息就是理解Windows背后机制的重要资料，因为符号蕴含了变量、函数、结构体等程序中关键信息的含义。有了符号，我们就可以“望文生义”，通过符号去大致了解变量、函数、结构题的用途，并最终帮助我们理解程序的意图。一个没有符号的程序，就像逻辑代数的真值表，程序work，但是人类却无法直接想清楚，这个程序是干啥的。

## 解题

官方WP写的贼简单，感觉意思是人人都会Win Pwn一样，其他WP参考如下：
