...
发现两个栈溢出，一个悬空指针，libc2.30不会利用，但别看题目开了NX，但是经过测试，这个保护并没有在这个板子上起作用。故可以用第一个栈溢出leak出栈地址，然后写shellcode到栈上：
...
add         = lambda index,size,data     :  (sla("choice > ","1"),sla("choice > ","1"),sla("index: ",str(index)), sla("size: ",str(size)) ,sla("content: ",str(data)))
delete      = lambda index               :  (sla("choice > ","1"),sla("choice > ","2"),sla("index: ",str(index)))
