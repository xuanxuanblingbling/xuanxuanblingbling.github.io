...
reason for some design decisions. Some design choices were motivated by other architecture restrictions which are not applicable to ours. In other cases, the fear to break others
...
所以最早是谁提出的，不太容易考察，但也没有太大的必要，但是大部分对此gadget的提及都是关于x64，包括blackhat的论文和蒸米的博文，当然x86这种以栈传参的指令集不需要研究怎么控制参数寄存器。但除了我们熟悉的x64:
...
这就很有意思了，就历史资料来看，通用gadget中**通用**的本意是x64架构下对于大部分应用程序来说的通用，因为只要是编译工具链是glibc的套件，就应该会带 **__libc_csu_init** 这个函数。但现在看起来，这个**通用**直接跨越了各种指令集，使其成为各种指令集下，各种应用程序中都包含的一段可从栈上控函数参数并调用的**真·通用**gadget。这神奇的一段gadget背后的本质道理到底是个啥呢？

## 我的理解

所以必然要回到 **__libc_csu_init** 这个函数的实现，虽然这个函数在ELF中，但其源码依然实现在glibc中，以2.27为例，其位于：[csu/elf-init.c](https://sourceware.org/git/?p=glibc.git;a=blob;f=csu/elf-init.c;h=da59b2c77b076c4cc33e003b2a25628773f9349c;hb=23158b08a0908f381459f273a984c6fd328363cb)，高版本libc后有所改动。
...
这导致调__init函数时必将保存参数寄存器到其他地方，由于__init为外部函数，编译器不知道其行为，所以如果没有人工干预，最终参数无论倒（二声）了几层寄存器，必然要存到栈上，导致这段gadget的存在似乎是一种必然性。但其实ROP最重要的是栈，只要打破参数是从栈上来的即可破坏此段gadget，所以，直接把三个参数拷贝成全局变量，调用时从全局变量中往出拿即可：
