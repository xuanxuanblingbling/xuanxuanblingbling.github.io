...
分析kysec保护进程，所以从通常的进程操作应该是死路。但是linux将进程的内存映射在proc伪文件系统中，所以尝试通过这里修改进程的内存进而使进程崩溃。分析可以修改的是sleep的got表，所以尝试先读取，got表地址如下：
...
```
dd if=/proc/11213/mem skip=4263944 count=8 bs=1 of=test.got
