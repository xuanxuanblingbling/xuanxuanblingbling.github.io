...
| 使用`dd`按照偏移读写文件               | [树莓派3B+刷openwrt安装mitmproxy折腾记录](https://xuanxuanblingbling.github.io/iot/2020/01/06/mitmproxy/)和重打包固件的修改目标文件系统。 |
...
| 使用root进程的漏洞提权                | [小路](https://wrlus.com/)有一次给我讲的某路由器提权方法     |
| 命令注入的入口可以是某些配置文件的内容 | 淼哥之前审出来的使用TF卡以及adb回复Getshell某设备    |
| 使用gdb劫持python控制流                | 平日Pwn题的调试                                              |
| 使用mount在文件系统中制造障眼法        | IoT设备上只读文件系统的修改                                  |
| 使用空格当路径的障眼法                 | 恶意混淆的障眼法域名                                         |
