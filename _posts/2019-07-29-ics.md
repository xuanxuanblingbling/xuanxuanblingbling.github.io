...
之前我知道的方法是：只有观察Pesudocode窗口中的函数地址，然后在IDA-View窗口中按G输入目标地址跳过去。但是对于代码的地址又没有方便的方法复制，只能手动输入很麻烦。最后找到一个插件：[LazyIDA](https://github.com/L4ys/LazyIDA)，安装方式就是LazyIDA.py放到插件目录里即可，我MAC的目录为：
